...
def info_dataset(dataset_name):
    return get_joint_info(dataset_name)

def landmark_vitpose(keypoints_results, body_part: str, dataset_name: str="coco_25"):
    if body_part == "right_hands_landmark" and dataset_name == "wholebody":
        temp = keypoints_results[112:133]
    elif body_part == "left_hands_landmark" and dataset_name == "wholebody":
        temp = keypoints_results[91:112]
    elif body_part == "face_landmark" and dataset_name == "wholebody":
        temp = keypoints_results[23:91]
    else:
        dataset_dict = info_dataset(dataset_name)
        index = get_index_body_part(body_part, dataset_name)
        temp = keypoints_results[index]

    if len(temp.shape) > 1:
        keypoints, conf = temp[:, :2], temp[:, 2]
        keypoints = [(int(keypoints[i][1]), int(keypoints[i][0])) for i in range(len(temp))]        
    else:
        keypoints, conf = temp[:2], temp[2]
        keypoints = (int(keypoints[1]), int(keypoints[0]))

    return keypoints, conf

def get_index_body_part(body_part: str, dataset_name: str="coco_25"):
    if body_part == "right_hands_landmark" and dataset_name == "wholebody":
    ...
    dataset_dict = info_dataset(dataset_name)
    return list(dataset_dict.keys())[list(dataset_dict.values()).index(body_part)]
...
def draw_keypoints(
    ...
    if img is None:
        ...
        if background == 'white':
            img = np.ones((img_shape_max, img_shape_max, 3), dtype=np.uint8) * 255
        elif background == 'black':
            img = np.zeros((img_shape_max, img_shape_max, 3), dtype=np.uint8)
    ...
    for index_subject, keypoints_subject in enumerate(keypoints):
        ...
        img = draw_points_and_skeleton(
            ...
            points_color_palette=points_color_palette,
            skeleton_color_palette=skeleton_color_palette,
            points_palette_samples=10,
            confidence_threshold=confidence_threshold,
        )
    return np.array(img[..., ::-1])  # Return RGB as original
